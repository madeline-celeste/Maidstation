...
public partial class SharedBodySystem
{
    ...
    private void OnBodyMapInit(Entity<BodyComponent> ent, ref MapInitEvent args)
    {
        ...
        EnsureComp<SurgeryTargetComponent>(ent); // Woundmed - Shitmed change. this is slop
    }
    ...
    private void MapInitBody(EntityUid bodyEntity, BodyPrototype prototype)
    {
        ...
        MapInitParts(rootPartUid, prototype, bodyEntity, rootPart); // Shitmed Change
    }
    ...
    /// </summary>
    private void MapInitParts(EntityUid rootPartId, BodyPrototype prototype, EntityUid body, BodyPartComponent rootPart) // Woundmed - add body,rootPart
    {
        ...
        while (frontier.TryDequeue(out var currentSlotId))
        {
            ...
            foreach (var connection in currentSlot.Connections)
            {
                ...
                var partSlot = CreatePartSlot(parentEntity, connection, childPartComponent.PartType, childPartComponent.Symmetry, parentPartComponent); // Woundmed - symmetry
    ...
    public virtual HashSet<EntityUid> GibBody(
        ...
        GibType gibType = GibType.Gib, // Woundmed
        GibContentsOption contents = GibContentsOption.Drop, // Woundmed
        List<string>? allowedContainers = null, // Woundmed
        List<string>? excludedContainers = null) // Woundmed
    {
        ...
        {
            // Woundmed - extra args provided
            _gibbingSystem.TryGibEntityWithRef(bodyId, part.Id, gibType, contents, ref gibs, playAudio: false,
